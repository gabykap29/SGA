...
from dependencies.checked_role import check_rol_admin, check_rol_moderate_or_admin
import uuid
...
def get_users(current_user: Dict = Depends(is_authenticated), is_authorized: bool = Depends(check_rol_moderate_or_admin), db_session=Depends(get_db)):
...
def get_user(id: str, current_user: Dict = Depends(is_authenticated), is_authorized: bool = Depends(check_rol_moderate_or_admin), db_session=Depends(get_db)):
    if not is_authorized:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def create_user(body: UserSchema, current_user: Dict = Depends(is_authenticated), is_authorized: bool = Depends(check_rol_admin), db_session=Depends(get_db)) -> dict[str, str]:
    if not is_authorized:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def update(id: str, body: UserSchema, current_user: Dict = Depends(is_authenticated), is_authorized: bool = Depends(check_rol_moderate_or_admin), db_session=Depends(get_db)) -> dict[str, str]:
    if not is_authorized:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def delete_user(id: str, current_user: Dict = Depends(is_authenticated), is_authorized: bool = Depends(check_rol_admin), db_session=Depends(get_db)) -> dict[str, str]:
    if not is_authorized:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
