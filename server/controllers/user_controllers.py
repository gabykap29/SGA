...
async def get_user(
    ...
    user = await user_model.get_user(id, db=db_session)
    if not user:
        raise HTTPException(
...
async def create_user(
    ...
    try:
        role_id = (
            uuid.UUID(str(body.role_id))
            if isinstance(body.role_id, str)
            else body.role_id
        )
...
async def update(
    ...
    edit = await user_model.edit_user(
        id_uuid,
        str(body.names),
        str(body.lastname),
        str(body.username),
...
async def delete_user(
    ...
    user = await user_model.delete_user(id, db=db_session)
    if not user:
        HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
